...
# Configuration
CONNECTION_STRING = os.getenv('POSTGRES_CONNECTION', "host=localhost port=5432 user=postgres password=example")
...
def calculate_metrics_postgresql(
    ...
    if 'tpep_pickup_datetime' in current_data.columns:
        current_data['pickup_datetime'] = pd.to_datetime(current_data['tpep_pickup_datetime'])
    elif 'lpep_pickup_datetime' in current_data.columns:
        current_data['pickup_datetime'] = pd.to_datetime(current_data['lpep_pickup_datetime'])
    ...
    logger.info(f"Metrics calculated - Drift: {prediction_drift:.4f}, "
...
if __name__ == "__main__":
    ...
    parser.add_argument('--reference-path', default='data/reference.parquet', help='Path to reference data')
    ...
    parser.add_argument('--start-date', default='2023-03-01', help='Start date for backfill')
    parser.add_argument('--days', type=int, default=30, help='Number of days for backfill')
