...
import boto3
import base64
import sys # For file structure/tree
import logging
from typing import Dict, List, Any
from models.predict import predict_single_trip, load_model, load_preprocessor # File structure
...
class LambdaHandler:
    ...
    def __init__(self):
        ...
        logger.info(f"LambdaHandler initialized - test_run: {self.test_run}")
        ...
        try:
            if self.run_id:
                ...
                try:
                    ...
                    logger.warning("Could not load preprocessor from models/dv.bin")
            else:
    ...
    def prepare_features(self, ride: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Course PU_DO feature - matches preprocess.py
    ...
    def send_prediction(self, prediction_event: Dict[str, Any], ride_id: str):
        ...
        if not self.test_run:
            try:
                self.kinesis_client.put_record( # Producer SENDS data TO streams
...
def get_handler():
    ...
    if lambda_handler_instance is None:
        lambda_handler_instance = LambdaHandler() # Create ONCE
    return lambda_handler_instance # Reuse same instance

# Class Method: LambdaHandler.lambda_handler()
