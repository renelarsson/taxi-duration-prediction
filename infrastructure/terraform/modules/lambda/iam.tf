...
# Create an IAM policy for kinesis processing
...
# Lambda permission for CloudWatch to trigger the function
...
# IAM policy for S3 access (model bucket)
resource "aws_iam_policy" "lambda_s3_role_policy" {
  ...
  # Current: Allows all S3 operations (s3:*)
  # Recommended: Only s3:GetObject and s3:ListBucket for security
policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
